...
另外，使用漏斗矿车吸取 1gt + 烧车也可以分离堆叠的物品实体，与前者结果相同而原理不同，这一点会在后续的分析中提及。
...
物品实体的 `Age` 属性是一个表示物品存在时间的计数器，但它并不会直接影响物品的分离顺序。物品的分离顺序主要受**实体自发运动顺序**（弹射式堆分离）或者**进入区段顺序**（漏斗矿车吸取 1gt + 烧车）的影响。
