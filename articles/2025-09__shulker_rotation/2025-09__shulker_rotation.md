...
“手性”实际的原理是潜影贝在乘坐船时，其**水平朝向**会被限制在**相对于船的水平朝向**的 `[-105°, +105°]` 范围内。该机制直到 25w36b 仍然存在，适用于所有类船载具（船、箱船、竹筏），并且可以通过特别构思的结构变换回原始朝向。“手性”用于描述该特性时，含义过于模糊，建议换成“隐偏航”（credit: Fallen_Breath）。
...
因此，建议使用“隐偏航”描述这一机制（credit: Fallen_Breath），既提及了“水平旋转”的属性，又暗示了这个属性的改变是不易观察的。在具体讨论机制时，直接使用属性原名“水平朝向”、`“yaw”` 或者 `“yRot”` 更佳，既能避免歧义，也方便与代码和游戏机制对应。
