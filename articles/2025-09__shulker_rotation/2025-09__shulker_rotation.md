...
“手性”实际的原理是潜影贝在乘坐船时，其**水平朝向**会被限制在**相对于船**的水平朝向的 `[-105°, +105°]` 范围内。该机制在新旧版本中均存在，并且可以通过特别构思的结构恢复原始朝向。“手性”用于描述该特性时，含义过于模糊，应该换成更确切的名称。
...
本文的讲解基于 Minecraft 1.18.1 和 1.21.5 版本，使用 Yarn 反混淆表。分析过程中有可能涉及断点调试等手段，所以更建议使用 Fabric mod 开发环境。


## 一、名词定义

- **水平旋转/水平朝向**：在这里指实体绕 y 轴旋转的角度，两者含义类似，所以可能会混用。Minecraft 中 y 轴为纵轴，代码中以 `yRot`(Mojang mapping) 和 `yaw`(Yarn mapping) 命名相关的旋转角度。
- **“手性”**：随意命名的特性名词，被 yellowxuuu 用于描述本文中涉及的特性。不清楚是否只有“左”、“右”的区别，仅在涉及原视频或他人原话时使用。
...
因此，更准确的称呼应当直接反映其机制，例如 {...}、{...} 等

在讨论潜影贝相关机制时，建议直接使用属性原名“水平朝向”、`“yaw”` 或者 `“yRot”` ，既能避免歧义，也方便与代码和游戏机制对应。
